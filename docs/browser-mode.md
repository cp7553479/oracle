...
- `--browser-thinking-time <light|standard|extended|extra-high|pro|heavy>`: set ChatGPT effort through the unified Intelligence picker, including `Advanced` → `Effort`. `extra-high` and `pro` are distinct; explicit `pro` fails closed if it cannot be confirmed. Other unmatched tiers keep the selection already active in ChatGPT. English, German, and Chinese effort labels are recognized. You can also set a default in `~/.oracle/config.json` via `browser.thinkingTime`.
...
- `--copy-profile <dir>`: escape hatch that copies a signed-in Chrome user-data directory (e.g. `"$HOME/Library/Application Support/Google/Chrome"`) to a throwaway profile and runs against it. It is the only flag that turns manual-login off, so prefer the default persistent manual-login profile; the copy fails fast with bounded rsync stderr when the required `Local State` or selected profile cannot be copied. macOS/Linux; requires `rsync`.
