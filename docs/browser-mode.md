...
- If an assistant response still times out (common with long Pro runs), Oracle marks the session as an incomplete capture, stores reattach/runtime diagnostics, and keeps enough browser metadata for `oracle session <id>` to recover the final answer. Visible ChatGPT rate-limit warnings are logged once without changing control flow; temporary-unavailable and authentication/challenge warnings remain included in error/session metadata. Increase `--browser-timeout` only when the browser session is truly unrecoverable.
