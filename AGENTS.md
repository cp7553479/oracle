...
- Browser manual-login is ALWAYS ON. Every browser run must reuse the persistent signed-in profile at `~/.oracle/browser-profile` (profile overrides from CLI, environment, and configuration are blocked). Legacy profile, cookie, copy, attach-running, remote-Chrome, and browser-tab arguments are compatibility no-ops: silently consume them without warnings, errors, blocking, or stopping the task. The mirror rule for conversation continuation: `--followup <sessionId|slug>` (saved ChatGPT browser sessions) and repeatable `--browser-follow-up <prompt>` are whitelisted and must stay functional through upstream refreshes — see `ALLOWED_FLAGS` in `src/cli/cliArgWhitelist.ts`; `--followup-model` stays a silent pass-through because browser follow-ups inherit the parent session's model, and API-only `resp_` references still fail closed. This is a hard requirement for all callers (hermes, openclaw, CLI, MCP): let the default apply, or pass `--manual-login` / `--browser-manual-login` explicitly. It is the fork default and intentionally diverges from upstream; do NOT revert it.
...
- Root CLI runs accept a whitelist only: prompt (`-p/--prompt`/`--message`/positional), files (`-f/--file` plus `--include/--files/--path/--paths`), AI+model selection (`-m/--model`, `--models`), conversation continuation (`--followup`, `--followup-model`, repeatable `--browser-follow-up`; browser follow-ups inherit the parent model), output/download paths (`--write-output`, `--output`), engine/manual-login flags, `--dry-run`, `--perf-trace`(+`--perf-trace-path`), internal plumbing (`--exec-session`, `--session`, `--status`), and meta flags. Every other root flag (effort, thinking-time, research, archive, render/copy, notify, timeout, provider, profile/cookie/attach/remote/tab, ...) is silently discarded WITH its values — no warning, no error. Engine/manual-login are always auto-appended; the AI choice IS `--model` (gpt-* → ChatGPT, gemini-_/claude-_). One browser task at a time; extra runs (including identical prompts) enter the queue silently and are never rejected; the queue wait has no timeout and prints nothing. The launched browser closes when the run, cancellation, or reattach harvest ends. Session/service subcommands (`status`, `session`, `serve`, `doctor`, ...) keep their full flags. `ORACLE_ALLOW_API_ENGINE=1` bypasses the whitelist and browser forcing (upstream test escape hatch only). With no model from CLI/config, root runs default to `--model latest` + Instant effort (thinking level `light`), not gpt-5.5-pro. New upstream root flags default to discarded until allowlisted in `src/cli/cliArgWhitelist.ts` and SPEC.md.
