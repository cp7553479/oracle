...
- Browser manual-login is ALWAYS ON (fork default, diverges from upstream): every browser run reuses the persistent signed-in profile at `~/.oracle/browser-profile` (override with `ORACLE_BROWSER_PROFILE_DIR` or `--browser-manual-login-profile-dir`). This is intentional — the upstream cookie-sync default (`--copy-profile` / system Chrome) is unreliable here and triggers `rsync failed copying Chrome profile (exit 23)` plus unsigned-in profiles. Do NOT revert this to upstream behavior, and do NOT pass `--copy-profile`. Callers (e.g. hermes) must let manual-login apply or pass `--manual-login` / `--browser-manual-login` explicitly; only `--copy-profile` opts out (and it is incompatible with manual-login).
