...
- CLI: agent-facing log output was audited line by line for signal. Removed bookkeeping noise (profile-lock/slot acquire-release churn, stale DevToolsActivePort cleanup detail, focus-emulation and cookie-skip notes, zero-count download scans, "Cleanup complete" telemetry, duplicate conversation-URL phase lines); demoted diagnostics to `--verbose` (shared-Chrome wait, stale-state clearing, prompt-ready repeats, UI-dismissal events); rewrote remaining lines to state only what callers need (`Signed in to ChatGPT`, `Prompt queued (N chars)`, `Thinking effort:`, one informational rate-limit notice, `Model verified: requested -> resolved`); fixed the footer model/mode separator (`GPT-5.6 Sol (browser)`).
