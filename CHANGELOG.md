...
- Browser: sync ChatGPT model/effort selection with the current unified Intelligence picker and default to `GPT-5.6 Sol` with Medium/standard effort. Explicit `gpt-5.5-instant` runs still select `GPT-5.5 Instant` with Instant/light effort, and `--browser-thinking-time` overrides either default. Add localized whole-word matching, distinct Extra High and Pro tiers, and evidence-backed model labels. If non-Pro model selection fails, continue with ChatGPT's current model and apply the requested effort best-effort; explicit Pro requests still fail closed.
