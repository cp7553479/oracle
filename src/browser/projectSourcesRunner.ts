...
function projectSourcesBrowserLeasePolicy(): { maxConcurrentTabs: 1; timeoutMs: 0 } {
  return { maxConcurrentTabs: 1, timeoutMs: 0 };
}
...
export async function runBrowserProjectSources(
  ...
  if (manualLogin) {
    tabLease = await acquireBrowserTabLease(userDataDir, {
      ...projectSourcesBrowserLeasePolicy(),
      logger,
...
export const projectSourcesBrowserLeasePolicyForTest = projectSourcesBrowserLeasePolicy;
