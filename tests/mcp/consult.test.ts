...
describe("summarizeModelRunsForConsult", () => {
  ...
  test("merges browser defaults from config for consult runs", () => {
    ...
    expect(config).toMatchObject({
      ...
      keepBrowser: true,
      manualLogin: true,
      manualLoginProfileDir: path.join(homedir(), ".oracle", "browser-profile"),
  ...
  test("ignores ordinary cookie sync when forced into manual-login mode", () => {
    const config = buildConsultBrowserConfig({
  ...
  test("allows image output over a remote browser service", async () => {
    ...
    try {
      ...
      expect(result.isError).not.toBe(true);
      expect(result.content[0]?.text).toContain("normal response wait");
    } finally {
