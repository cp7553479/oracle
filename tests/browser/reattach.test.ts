...
describe("manual-login cookie sync recovery", () => {
  test("closes a replacement browser when connecting to it fails", async () => {
    const profileDir = await mkdtemp(path.join(os.tmpdir(), "oracle-reattach-connect-failure-"));
    try {
  ...
  test("closes a replacement browser when setup fails before answer polling", async () => {
    const profileDir = await mkdtemp(path.join(os.tmpdir(), "oracle-reattach-setup-failure-"));
    try {
      const expected = new Error("runtime setup failed");
      const kill = vi.fn(async () => {});
      const close = vi.fn(async () => {});
      const launchChrome = vi.fn(async () => ({ port: 9222, kill }));
      const connectToChrome = vi.fn(async () => ({
