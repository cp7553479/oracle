...
describe("ensureModelSelection composer-pill wait", () => {
  ...
  it("gives up once the button-wait deadline passes", async () => {
    ...
    await expect(
      ensureModelSelection(Runtime, "Pro", logger, "select", {
        buttonWaitMs: 5,
        buttonPollMs: 1,
      }),
    ).resolves.toMatchObject({
    ...
    expect(logger).toHaveBeenCalledWith(
      'Model picker: selector unavailable for "Pro"; continuing with current ChatGPT model.',
    );
  });
  ...
  it("continues with current model and logs available options when no match exists", async () => {
    ...
    await expect(
      ensureModelSelection(Runtime, "gpt-5.5", logger, "select"),
    ).resolves.toMatchObject({
